  ...
  <div class="featured-content">
    <div class="row-fluid">
     <div class="addashmain" style="background-color:#d3bfce;">
	      <h2 style="color: black">Welcome To Student Dashboard</h2>
	      <h3 style="color: black">Hi, <?php echo Session::get('userusername')?></h3>
	      <?php
            if(!$msg)
              echo "<p style='color:mintcream;'><i>You Are Not Connected Any Group.Please Connect Now</i></p>";
            	...
            	echo "<p><i>You Are Connected to the ".$i." Group(s)</i></p>";
            }
	      ?>
	      <span style="margin-left: 850px;">
	      <a href="userconnectgroup.php"><input type="submit" name="cg" value="Connect To Group" name="" style="background-color: purple"></a>
    <?php
     if(isset($_GET['usaction']) && $_GET['usaction']=='uslogout')              
               Session::destroy(); 
    ?>
	      <a href="?usaction=uslogout"><input type="submit" name="lg" value="Log Out" name="" style="background-color: purple"></span></a>
	      <br>
     </div>
    </div>
   </div>
	    ...
	    <div class="row-fluid eachgr" style="background-color:#a881a0;">
	      ...
	      <p style="margin-top: -12px;"><strong>Current Time: </strong><span style="color:mintcream"><?php echo $fm->DateFormat($st);?></span></p>
           ...
           <p style="margin-top: -12px;">
            ...
            <?php
                 if($ck1=='f') {?>
           <a href="examhall.php?egid=<?php echo $group_id ;?>&gtok=<?php echo $group_token;?>"><input type="submit" name="viewgrp" value="Enter Exams" style="background-color: purple;color: white"></a>
            <?php } ?>
	          <?php 
                 if($ck=='f') {?>
                 <?php
                 ...
                 if(!$cnfm)  { ?>
                   <input onclick = "confirm('Result Not Published Yet')" type="submit" name="addquestion" value="Result" style="background-color: purple;color: white">
                <?php } else{
                 ?>
                  <a href="userresult.php?resid=<?php echo $userid;?>&regpid=<?php echo $group_id;?>&tgtq=<?php echo $tgtq;?>"><input type="submit" name="addquestion" value="Result" style="background-color: purple;color: white"></a>
               <?php  } } ?> 
	      </div>
	   </div>
	   <br>
